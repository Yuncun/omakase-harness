...
set -euo pipefail
# A leaked GIT_DIR/GIT_WORK_TREE/GIT_COMMON_DIR (exported for ANOTHER repo by a wrapper or a
# parent hook) would misdirect every git call below — healing the wrong tree.
# This script's repo is always the one it runs in: resolve from cwd only.
