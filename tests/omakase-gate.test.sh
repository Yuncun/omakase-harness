...
R="$TMP/disabled"; newrepo "$R"
(
  cd "$R"
  common="$(cd "$(git rev-parse --git-common-dir)" && pwd)"
  mkdir -p "$common/omakase"
  printf 'noisy\n' > "$common/omakase/disabled-gates"
  out="$("$GATE" noisy --step 'exit 7' 2>&1)"; rc=$?
  if [ $rc -eq 0 ] && echo "$out" | grep -q "disabled via omakase"; then
    pass "listed gate skips visibly (exit 0)"
  else
    fail "listed gate skips visibly (rc=$rc out=$out)"
  fi
  "$GATE" other --step 'exit 7' >/dev/null 2>&1; rc2=$?
  [ $rc2 -eq 7 ] && pass "unlisted gate still runs" || fail "unlisted gate still runs (rc=$rc2)"
  : > "$common/omakase/disabled-gates"
  "$GATE" noisy --step 'exit 7' >/dev/null 2>&1; rc3=$?
  [ $rc3 -eq 7 ] && pass "re-enabled gate runs again" || fail "re-enabled gate runs again (rc=$rc3)"
)
