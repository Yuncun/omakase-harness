...
Status: LOCKED 2026-07-02. Produced from a 3-design judge panel (simplicity-first won
9/9/9/7 from both judges) plus grafts from the two runner-up designs. This document is
the contract for the v2 implementation; when it and the code disagree during the build,
this document wins until amended. §8 was decided provisionally by Claude under the
owner's standing approval (owner AFK) — it is deliberately the cheapest decision in the
design to reverse (one mapping-table row, no state impact).
...
machines stays dependency-free POSIX `sh`, generated by the binary from templates:
`ensure-present.sh`, `verify-overlay.sh`, `install-guards.sh`, the hook stubs, and
`omakase-gate.sh` + gates in the payload.
...
The sh gate primitive (`payload/.omakase/bin/omakase-gate.sh`) gains ONE check right
after the existing `OMAKASE_SKIP_<NAME>` env bypass:
...
2. **Round-trip tests:** bash-initialized repo → v2 `remove` reverses byte-exactly;
   v2-written state → v1 sh hook readers parse identically; ≤5-column writer test on
   placed.tsv.
...
|---|---|---|
| 0 | Compat scaffolding: round-trip tests, ≤5-col writer test, golden v1 fixtures | tests exist and pass against v1 bash |
