---
name: status
description: Show what omakase harness is installed in the current repo and what runs on which git hook — the inventory (committed / injected / global), the hook wiring, the recent-runs scorecard, and the hidden paths. Read-only. Use when asked "omakase status", "what harness is installed", "show the harness", or "what gates run here".
