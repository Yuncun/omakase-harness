...
#      from cwd only (verify-only run: OMAKASE_SKIP_GATES=1 omakase hook
#      pre-commit — the skip drops the gates, never the verify; heal: hook
#      post-checkout).
...
#      E3 a recorded verdict row (omakase record) lands in the cwd repo's
#         ledger, and the leaked repo's git dir stays untouched
...
#      corruption class: a runner's run-time sync + omakase's own re-arm both
...
#      A2 gate wiring is the ONE init-written snapshot manifest under the shared
#      git dir: two worktrees run the same shared-snapshot gate on commit and
#      NEITHER writes to .git/hooks — the pre-#98 scheme raced here because both
...
# permanent .git/hooks dispatchers.
...
rm -f "$REPOA/$REL"
OUT="$( cd "$REPOA" && GIT_DIR="$REPOB/.git" GIT_WORK_TREE="$REPOB" GIT_COMMON_DIR="$REPOB/.git" OMAKASE_SKIP_GATES=1 "$OMAKASE" hook pre-commit 2>&1 )"; RC=$?
...
if ( cd "$REPOA" && OMAKASE_SKIP_GATES=1 "$OMAKASE" hook pre-commit ) >/dev/null 2>&1; then
  pass "E2: overlay verifies clean after the heal"
else
  fail "E2: overlay still failing verification after the heal"
fi

echo "== E3: a recorded verdict row lands in the cwd repo under the leaked env =="
( cd "$REPOA" && GIT_DIR="$REPOB/.git" GIT_WORK_TREE="$REPOB" GIT_COMMON_DIR="$REPOB/.git" "$OMAKASE" record leaktest ) >/dev/null 2>&1 \
  || fail "E3: record run exited non-zero"
...
echo "== A2: two worktrees share the write-once hooks; neither commit rewrites them =="
# Gate wiring now lives in the ONE init-written snapshot manifest under the shared
# git dir, so both worktrees run the same gates — the pre-#98 scheme raced here
# because each worktree kept rewriting the one shared hook. What must hold: each
# worktree's commit fires the shared-snapshot gate, and NEITHER writes a byte to
# .git/hooks (the write-once invariant).
WTA="$TMP/wt-alpha"
...
printf '%s' "$OUTM" | grep -q 'example gate passed' \
  && pass "A2: the main checkout fired the shared-snapshot gate" \
  || fail "A2: main-checkout commit did not fire the gate ($OUTM)"
printf '%s' "$OUTA" | grep -q 'example gate passed' \
  && pass "A2: the linked worktree fired the shared-snapshot gate" \
  || fail "A2: worktree commit did not fire the gate ($OUTA)"
