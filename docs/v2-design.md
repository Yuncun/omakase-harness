...
`verify-overlay.sh`, `install-guards.sh`, `omakase-gate.sh`, and the hook stubs are
POSIX `sh`; **`ensure-present.sh` is bash** (3.2 floor — it uses `${var:0:n}`). v2
templates keep each script's current interpreter; a deliberate port of
`ensure-present.sh` to POSIX sh is allowed only under the Phase 0 compat suite
(`tests/state-readers.test.sh`).
