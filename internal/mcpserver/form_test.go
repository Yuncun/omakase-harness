...
// emit one Op each carrying the group children for the apply loop.
...
// ApplyOps applies a batch of ops to a copy of a screen's items without ever
// touching the caller's slice — kept for tests that want to assert on the
// resulting Enabled/PartialOff state.
...
// stateByKey is the flat on/off lookup menuMessage counts gates and files
// from: one entry per toggleable leaf, keyed the same way BuildForm keys its
// leaf fields.
