#!/usr/bin/env bash
# FROZEN v1 REFERENCE (moved from bin/init.sh by the v2 port): parity oracle for the Go binary + the shim's fallback body; kept byte-identical except this note and the SCRIPT_DIR line (retargeted UP one dir so its bin/ siblings — lib-*.sh, the install_script sources — and ../payload still resolve from bin/legacy/), until Phase 7 retires it.
...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # bin/legacy/ -> bin/: keeps every $SCRIPT_DIR-relative source (lib-*.sh, ../payload, install_script) resolving as it did from bin/
