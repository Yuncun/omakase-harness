...
# omakase-harness — resolution of the omakase Go binary. Sourced by
# bin/init.sh, bin/status.sh, bin/remove.sh, hooks/session-start.sh. NOT
...
# `set -u`. Callers must set $HERE (their bin/ directory) before calling
# resolve_omakase.
#
...
# There is no network tier: a plugin-only install refuses with the brew line
# instead of downloading a binary (issue #182 — the Anthropic first-party
# plugin pattern). Resolution is local and offline, always.
