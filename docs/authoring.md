...
as `gate:` blocks in `omakase.manifest` (see [Reference](reference.md#manifest)) and ship only
your own gate scripts. If a gate's `run:` names a payload script (`.omakase/…` or `gates/…`)
...
A gate is one `gate:` block in `omakase.manifest` (see [Concepts](concepts.md#gates)). A
block opens with `gate: <name>` and carries these keys:
