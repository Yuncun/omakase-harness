...
func Scan(repo *state.Repo, home, host string) []Entry {
	...
	out = append(out, parentLayer(repo, home, host)...)
...
// parentLayer is the CLAUDE.md files in the directories between the repo and
// $HOME. Claude Code loads every CLAUDE.md on the walk from the working
// directory up, so a file two directories above the repo steers every session
// inside it — the least visible always-on layer there is, and the one a
// context page must not miss. Copilot does not walk up, so these are
// claude-only. ~/.claude/CLAUDE.md is the global layer, handled separately.
func parentLayer(repo *state.Repo, home, host string) []Entry {
	if home == "" {
		return nil
	}
	// Both sides resolved: on macOS $HOME and the repo path routinely differ
	// in whether /var is spelled /private/var, and an unresolved comparison
	// would silently skip the whole walk.
	home = realPath(home)
	var out []Entry
	dir := filepath.Dir(realPath(repo.Root))
	for strings.HasPrefix(dir, home+string(filepath.Separator)) || dir == home {
		abs := filepath.Join(dir, "CLAUDE.md")
		if n := fileBytes(abs); n > 0 {
			display := abs
...
// itself across host-specific filenames. Entries are grouped by resolved
// path first: CLAUDE.md as a symlink to AGENTS.md (or the reverse) is the
// standard way to serve both hosts from one file, and listing it twice would
// double-count the bytes and then report a "~100% overlap" between a file
// and itself.
func projectLayer(repo *state.Repo, host string) []Entry {
	...
	for _, rel := range rels {
		...
		g.names = append(g.names, rel)
		for _, h := range hostsFor(rel) {
...
func injectedRuleLayer(repo *state.Repo, injected map[string]bool, host string) []Entry {
	...
	for _, dir := range []string{".github/instructions", ".claude/rules"} {
		for _, rel := range mdFilesUnder(repo.Root, dir) {
			...
			// A rule scoped by frontmatter globs (Claude `paths:`, Copilot
			// `applyTo:`) loads only when the session touches a matching
			// file — never a per-turn cost.
			want := TierAlways
			if e.Scope = scopeGlobsOf(abs); len(e.Scope) > 0 {
				want = TierOnDemand
			}
			e.Tier = tierFor(e.Hosts, host, want)
			out = append(out, e)
		}
	}
	return out
