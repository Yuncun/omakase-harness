#!/usr/bin/env bash
# FROZEN v1 REFERENCE (moved from bin/remove.sh by the v2 port): parity oracle for the Go binary + the shim's fallback body; kept byte-identical except this note and the SCRIPT_DIR line (retargeted UP one dir so lib-lefthook.sh + ../payload still resolve from bin/legacy/), until Phase 7 retires it.
...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # bin/legacy/ -> bin/: keeps $SCRIPT_DIR-relative sources (lib-lefthook.sh, ../payload) resolving as they did from bin/
