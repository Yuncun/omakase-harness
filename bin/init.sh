...
# Insert a guard into the enforcement stubs (pre-commit, pre-push) that runs
# verify-overlay.sh before lefthook and blocks when the overlay is gone. The guard is
# inert once the harness is removed ($COMMON/omakase deleted). post-checkout is
# deliberately NOT guarded: its job (ensure-present) is the self-heal path and must
# keep running. Marked block, strip-then-insert, so re-runs stay idempotent. A manual
# `lefthook install` regenerates the stubs and drops the guard until the next init.
